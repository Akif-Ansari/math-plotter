...
export default function Header({
  ...
  return (
    <header className={`h-12 md:h-16 border-b px-3 md:px-4 flex items-center justify-between select-none shadow-lg z-20 transition-colors duration-200 ${
      ...
      <div className="flex items-center gap-2 md:gap-3 min-w-0">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h1 className={`font-bold text-base md:text-lg tracking-tight bg-clip-text text-transparent ${
              isDarkTheme
          ...
          <p className={`text-xs font-mono hidden md:block ${isDarkTheme ? 'text-zinc-400' : 'text-zinc-500'}`}>
      ...
      <div className="hidden md:flex items-center gap-2">
        <div className="relative">
          <select
            ...
            defaultValue=""
            className={`text-xs font-medium px-3 py-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-[#1DB954] cursor-pointer shadow-sm transition ${
              isDarkTheme
        ...
        <div className="fixed inset-0 z-50 md:hidden" onClick={() => setMenuOpen(false)}>
          ...
          <div
            className={`absolute top-12 right-2 w-64 border rounded-xl shadow-2xl animate-slide-up overflow-hidden ${
              isDarkTheme ? 'bg-zinc-900 border-zinc-700 text-white' : 'bg-white border-zinc-200 text-zinc-900'
            }`}
          ...
          >
            <div className={`flex items-center justify-between px-4 py-3 border-b ${
              ...
              <span className={`text-xs font-semibold ${isDarkTheme ? 'text-zinc-300' : 'text-zinc-800'}`}>
                Tools &amp; Presets
              </span>
              <button onClick={() => setMenuOpen(false)} className="p-1 text-zinc-400 hover:text-zinc-700">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {onOpenTable && (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs rounded-lg transition ${
                    isDarkTheme ? 'text-zinc-200 hover:bg-zinc-800' : 'text-zinc-700 hover:bg-zinc-100'
                  }`}
                >
